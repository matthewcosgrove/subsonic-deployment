...
SCRIPT_DIR_STATE=$SCRIPT_DIR/state-sandbox
mkdir -p $SCRIPT_DIR_STATE


pushd $SCRIPT_DIR/terraform > /dev/null
terraform init 
terraform apply -input=false -auto-approve -state=$SCRIPT_DIR/terraform/state-sandbox/terraform.tfstate

elastic_ip=$(terraform output -state=$SCRIPT_DIR/terraform/state-sandbox/terraform.tfstate elastic_ip)
az=$(terraform output -state=$SCRIPT_DIR/terraform/state-sandbox/terraform.tfstate default_subnet_az) 
default_security_group=$(terraform output -state=$SCRIPT_DIR/terraform/state-sandbox/terraform.tfstate default_security_group)
subnet_id=$(terraform output -state=$SCRIPT_DIR/terraform/state-sandbox/terraform.tfstate default_subnet_id)
subnet_cidr=$(terraform output -state=$SCRIPT_DIR/terraform/state-sandbox/terraform.tfstate default_subnet_cidr)
subnet_gw=$(awk -F"." '{print $1"."$2"."$3".1"}'<<<$subnet_cidr)
subnet_new_ip=$(awk -F"." '{print $1"."$2"."$3".11"}'<<<$subnet_cidr)
...
echo "AWS_ELASTIC_IP=$elastic_ip" > $SCRIPT_DIR_STATE/elastic-ip
echo "AWS_ELASTIC_IP=$elastic_ip"
echo "written to $SCRIPT_DIR_STATE/elastic-ip"
cat $SCRIPT_DIR_STATE/elastic-ip
popd > /dev/null
